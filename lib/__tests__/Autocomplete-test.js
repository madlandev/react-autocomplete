...
function AutocompleteComponentJSX(extraProps) {
  return (
    <Autocomplete
      ...
      autoHighlight={(itemValue: string, inputValue: string): boolean => itemValue.toLocaleLowerCase().includes(inputValue.toLocaleLowerCase())}
...
describe('Autocomplete acceptance tests', () => {
  ...
  it('should not highlight top match when `props.autoHighlight` is undefined', () => {
    const tree = mount(AutocompleteComponentJSX({
